...
import { useQuickTournament } from "../lib/queries"
import { quickTournamentApi } from "../lib/api"

export default function QuickTournamentDraw() {
  const { id } = useParams()
  const navigate = useNavigate()
  const { data: tournament } = useQuickTournament(id)
  ...
  if (tournament && (tournament.status === "active" || tournament.status === "completed")) {
    navigate(`/quick-tournament/bracket/${id}`, { replace: true })
    return null
  }
  ...
  useEffect(() => { if (phase === "revealing") setRevealStep(0) }, [matchIdx])
    ...
    <div className="min-h-screen bg-pitch-900 flex flex-col items-center justify-center p-6"
      ...
      <div className="flex items-center gap-3 mb-10">
        ...
        <div>
          ...
          <p className="text-xl font-extrabold text-white">{tournament?.name || "Quick Tournament"}</p>
        </div>
      </div>
      ...
      {phase === "revealing" && currentMatch && (
        <div className="w-full max-w-lg text-center">
          <p className="text-xs text-slate-500 uppercase tracking-widest mb-8">Match {matchIdx + 1} of {r1Matches.length}</p>
          <div className="bg-pitch-800 border border-surface-border rounded-2xl px-8 py-10 space-y-6"
            ...
            <div style={{ opacity: revealStep >= 1 && !fadingOut ? 1 : 0, transform: revealStep >= 1 ? "translateY(0)" : "translateY(20px)", transition: fadingOut ? "opacity 0.3s ease" : "opacity 0.5s ease, transform 0.5s ease" }}>
            ...
            <div style={{ opacity: revealStep >= 2 && !fadingOut ? 1 : 0, transition: fadingOut ? "opacity 0.3s ease" : "opacity 0.4s ease" }}>
              <p className="text-lg font-bold text-slate-500">VS</p>
            </div>
            <div style={{ opacity: revealStep >= 3 && !fadingOut ? 1 : 0, transform: revealStep >= 3 ? "translateY(0)" : "translateY(-20px)", transition: fadingOut ? "opacity 0.3s ease" : "opacity 0.5s ease, transform 0.5s ease" }}>
          ...
          <button onClick={() => setPhase("done")} className="mt-6 text-xs text-slate-500 hover:text-white transition-colors underline decoration-dotted">
            Skip animation →
          </button>
        </div>
      )}
          ...
          <div className="space-y-4 mb-8 max-w-sm mx-auto">
            {r1Matches.map((m) => (
              <div key={m.id} className="flex items-center justify-between bg-pitch-800 rounded-xl px-4 py-3 border border-surface-border">
                ...
                <span className={`text-sm font-medium ${m.player2Name ? "text-white" : "text-slate-500 italic"}`}>{m.player2Name || "BYE"}</span>
              </div>
            ))}
          </div>
          <button
            onClick={async () => { setStarting(true); await quickTournamentApi.start(id); navigate(`/quick-tournament/bracket/${id}`) }}
            disabled={starting}
