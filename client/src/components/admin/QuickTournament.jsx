...
import { useQuickTournaments, useCreateQuickTournament, useSetQuickTournamentPlayers, useDeleteQuickTournament } from "../../lib/queries"
...
function CreateForm({ onClose }) {
  ...
  const createTournament = useCreateQuickTournament()
  const setPlayers      = useSetQuickTournamentPlayers()
  ...
  return (
    <div className="card p-5 space-y-4 mb-5">
      <h3 className="text-sm font-semibold text-white">Create Quick Tournament</h3>
...
export default function QuickTournamentAdmin() {
  const navigate = useNavigate()
  const { data: tournaments = [], isLoading } = useQuickTournaments()
  const deleteTournament = useDeleteQuickTournament()
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">Quick Tournaments</h2>
        </div>
        {!showCreate && (
      ...
      {isLoading ? (
        ...
        <div className="card px-5 py-10 text-center">
          ...
          <p className="text-slate-400 text-sm">No quick tournaments yet</p>
            ...
            return (
              <div key={t.id} className="card flex items-center gap-4 px-5 py-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {t.status === "draw" && (
                        <button onClick={() => navigate(`/quick-tournament/draw/${t.id}`)}
                      ...
                      {(t.status === "active" || t.status === "completed") && (
                        <button onClick={() => navigate(`/quick-tournament/bracket/${t.id}`)}
