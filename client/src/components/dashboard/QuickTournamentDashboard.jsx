...
import { useQuickTournamentCurrent, useQuickTournamentTopScorers } from "../../lib/queries"
...
export default function QuickTournamentDashboard({ onPlayerClick }) {
  const { data: tournament, isLoading } = useQuickTournamentCurrent()
  const { data: scorers = [] } = useQuickTournamentTopScorers()
  const quickScorerRankChanges = useRankChanges("quick-golden-boot", scorers.map(s => s.id))
  ...
  if (!tournament) {
    return (
      <div className="card px-6 py-14 text-center">
        ...
        <p className="text-slate-400 font-medium">No Quick Tournament yet</p>
    ...
    <div className="card overflow-hidden relative">
      <div className="px-5 py-4 border-b border-surface-border flex items-center justify-between">
        <div className="flex items-center gap-3">
          <img
            src={view === "fixtures" ? quickLogo : goldenBootLogo}
            alt={view === "fixtures" ? "Quick Tournament" : "Golden Boot"}
            className="w-9 h-9 object-contain flex-shrink-0"
          />
          <div>
            <p className="section-label mb-0.5">Quick Tournament</p>
      ...
      {view === "scorers" && (
        scorers.length === 0 ? (
          <div className="px-5 py-10 text-center">
            ...
            <p className="text-sm text-slate-500">No Quick Tournament goals logged yet</p>
          </div>
        ) : (
                    ...
                    >
                      <td className="py-3 px-3 text-center">
                        {isFirst
                          ...
                          : <span className="flex items-center justify-center gap-0.5"><span className="text-slate-500 text-sm">{idx + 1}</span><RankBadge change={quickScorerRankChanges[scorer.id]} /></span>
                        }
                      </td>
